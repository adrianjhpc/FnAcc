...
__global__ void axpby_f64_kernel(double alpha, double beta, const double *a,
                                 const double *b, double *c, long long n) {
...
int main(int argc, char **argv) {
  ...
  for (long long i = 0; i < n; ++i) {
    ...
    b[i] = 100.0 - 0.25 * static_cast<double>(i + 1);
    c[i] = 0.0;
  }
  ...
  axpby_f64_kernel<<<grid, block>>>(alpha, beta, da, db, dc, n);
  ...
  int errors = 0;
  for (long long i = 0; i < n; ++i) {
    double expected = alpha * a[i] + beta * b[i];
  ...
  std::printf("cuda_axpby_f64,%lld,1,%d,%.8e,%.8e,%d\n", n, reps, avgSeconds,
              gbps, errors);
