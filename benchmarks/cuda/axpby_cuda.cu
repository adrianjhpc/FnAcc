...
__global__ void axpby_kernel(float alpha, float beta, const float *a,
                             const float *b, float *c, long long n) {
...
int main(int argc, char **argv) {
  ...
  for (long long i = 0; i < n; ++i) {
    ...
    b[i] = 100.0f - 0.25f * static_cast<float>(i + 1);
    c[i] = 0.0f;
  }
  ...
  axpby_kernel<<<grid, block>>>(alpha, beta, da, db, dc, n);
  ...
  std::printf("cuda_axpby,%lld,1,%d,%.8e,%.8e,%d\n", n, reps, avgSeconds,
              gbps, errors);
