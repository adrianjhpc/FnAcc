program matmul_2d_f64_openmp
  ...
  ! Warm-up execution on CPU with OpenMP.
  !$omp parallel do collapse(2) private(i, j, p, acc) schedule(static)
  do j = 1, m
    do i = 1, n
      acc = 0.0
      do p = 1, k
  ...
  call print_result("openmp_matmul_2d_f64", n, m, reps, elapsed, flops_per_rep, errors)
